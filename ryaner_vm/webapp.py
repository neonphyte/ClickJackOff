    ...
    return """
         <html>
            ...
            <body>
                ...
                <br>
                <a href="https://www.7-zip.org/a/7z2409-x64.exe" target="_blank">7z download link. SAFE for initial scan but MALICIOUS on falcon sandbox</a>
                <br>
                <a href="http://www.stock888.cn/" target="_blank">zk malicious link confirm plus chop. MALICIOUS; DO NOT CLICK THIS AT ALL</a>
            </body>
        </html>
    """

# Main function for receiving URL to check for download indicators & perform analysis, before sending results back to client
...
# 1. Initial Downloadable URL analysis; Attach risk level to URL based on file extension & headers
# Return to client if no download indicators detected
def analyze_download_indicators(url):
    try:
        ...
        # End analysis if no download indicators detected 
        return {
...
# 2a. Send URL to VirusTotal for analysis
...
# Helper function to extract JSON content from mixed output string in step 2b.     
...
# 2b. Sends URL into Hybrid analysis's falcon sandbox for analysis
def check_falconsandbox_download(url):
    try:
        ...
        # Parse JSON submission response to get sha256 value; for retrieving analysis results later
        try:
        ...
        # Wait briefly for analysis to begin (adjust time as needed)
        time.sleep(10)  # Wait 10 seconds for initial analysis

        # Get analysis report results using sha256
